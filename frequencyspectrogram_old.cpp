...
public:
    FrequencyScope(int samplerate) : bufferSize(samplerate), FFT(samplerate)/*, freqBuf(new float[samplerate])*/ {
        this->primitive(Mesh::LINE_STRIP);
        numBins = bufferSize / 2 + 1;
        for (int i = 0; i < bufferSize; i++) {
            // this->vertex((i / static_cast<float>(bufferSize)) * 2.f - 1.f, 0);
            buffer.push_back(0.f);
            freqBuf.push_back(0.f);
            // freqBuf[i] = 0;
        }
        for (int i = 0; i < numBins; i++) {
            this->vertex((i / static_cast<float>(numBins)) * 2.f - 1.f, 0);
        }
    }
    ...
    void writeSample(float sample) {
        ...
        buffer[bufferSize - 1] = sample;
        freqBuf[bufferSize - 1] = sample;
    }

    void update() {
        std::cout << "UPDATING \n";
        //FFT.forward(&freqBuf[0]);
        for (int i = 0; i < numBins; i++) {
            gam::Complex<float> c;
            if (i == 0) c(freqBuf[0], 0);
            else if (i == bufferSize / 2) c(freqBuf[bufferSize - 1], 0);
            else c(freqBuf[i * 2 - 1], freqBuf[i * 2]);

            float val = c.mag();

            this->vertices()[i][1] = val;
        }
    }
protected:
    int bufferSize;
...
public:
    ...
    FrequencyScope myFreq{ static_cast<int>(AudioIO().framesPerSecond()) };
...
// Set sampling rate for Gamma objects from app's audio
    ...
    void onSound(AudioIOData& io) override {
        ...
        while (io()) {
            ...
            myFreq.writeSample((io.out(0) + io.out(1)) / 2.f);
        }
    }
    ...
    void onDraw(Graphics& g) override {
        ...
        g.translate(Vec2f(0, 0.5f));
        g.draw(myScope);
        g.translate(Vec2f(0, -1.f));
        g.draw(myFreq);
        // Draw GUI
        imguiDraw();
    }
