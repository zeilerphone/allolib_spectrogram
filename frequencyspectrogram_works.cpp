...
public:
        ...
        stft(fft_window_size, fft_window_size / 4, 0, gam::HANN, gam::MAG_FREQ)
    {
        ...
        for (int i = 0; i < numBins / 2 + 1; i++) {
            this->vertex(i / static_cast<float>(numBins) * 4.f - 1.f, 0.0);
        }
        spectrum.resize(numBins / 2 + 1);
        ref = 0.55;
    }
    ...
    void writeSample(float sample) {
        if (stft(sample)) {
            for (int i = 0; i < numBins / 2; ++i) {
                ...
                spectrum[i] = 2 * log10(32 * (val+0.001) + ref) ;
                //cout << val << ", " << max << endl;
                // 0.03
            }
        }
...
public:
    Rectangle(float w, float h) : width(w), height(h) {
        this->reset();
        this->primitive(Mesh::LINE_LOOP);
        this->vertex(-width / 2, height / 2, 0.0);
        this->vertex(width / 2, height / 2, 0.0);
        this->vertex(width / 2, -height / 2, 0.0);
        this->vertex(-width / 2, -height / 2, 0.0);
    }
...
public:
    ...
    Rectangle scopeBound{ 1.8f,1.f }; 
    Rectangle freqBound{ 1.8f,1.f };
...
// Set sampling rate for Gamma objects from app's audio
    ...
    void onAnimate(double dt) override {
        ...
        imguiEndFrame();
        myScope.update();
        //myFreq.update(); // maybe limit the calls
    }
